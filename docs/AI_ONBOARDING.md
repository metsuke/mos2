...
**Documentos relacionados:** AGENTS.md, docs/METHODOLOGY.md, docs/ENVIRONMENTS.md, docs/VERSIONING.md, docs/STYLE_GUIDE.md, docs/specs/00-OVERVIEW.md
...
7. docs/specs/00-OVERVIEW.md y, según la tarea, SSS / SEC / ICD / SRS / SDD / TEST / SRelD
8. docs/USER_MANUAL.md y docs/man/ si la tarea afecta al uso
9. Código solo después de saber qué norma aplica
...
- Encabezados de documentación **sin numeración** (`## Título`, no `## 1. Título`). Si el archivo del repo aún tiene números, entregar el documento completo ya sin números.
- Si la versión nueva de un doc es más corta que la del repo, comprobar que no se pierde norma e **informarlo**.
...
2. Comprobar impacto en SEC / SSS / ICD / SRS.
3. Respetar STYLE_GUIDE y seguridad de imports.
4. Plan por fases con commits atómicos.
...
| Solo documentación / onboarding | No bump | vX.Y.Z-docs (opcional) |
...
- windows/git-bash: priorizar py -m poetry (poetry.exe puede dar Permission denied).
- windows/wsl: clone en filesystem Linux; rechazo si la raíz está bajo /mnt/<letra>/ (guard con SCRIPT_DIR definido antes del check).
...
2. ¿Normas SEC/SSS/ICD respetadas?
...
6. ¿Entrega en un bloque / un paso?
...
Si entra en conflicto con SEC o SSS, prevalecen SEC y SSS; luego hay que actualizar este onboarding.