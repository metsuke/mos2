...
**Baseline de referencia:** v0.2.5 (texto 1.3 conservado); producto actual v0.2.7 / árbol v0.2.8  
**Estado:** Normativo  
...
| Licencia | GPL-3.0 |
| Estado | Alpha |
| Baseline actual | v0.2.5 (secciones 1.3) / producto v0.2.7 |
...
## Ampliación normativa 1.4 (producto 0.2.7 / árbol 0.2.8)

Este apartado **no sustituye** las secciones 1.3. Actualiza el estado de la reserva «campaña 07» y añade normas que el código ya cumple.

### Baseline

| Campo | Valor |
|-------|-------|
| Producto Poetry / main | v0.2.7 |
| Árbol hacia tag | v0.2.8 (pendiente de pruebas humanas) |

### Definición añadida

Además de lo ya listado en 1.3, el sistema proporciona:

- Apps instalables (path local o repo git), ámbito usuario o sistema
- Tareas e hilos locales (GTD) y worker de sesión
- Fachada de modelos `iarouter` (off por defecto)
- Comando `red` de diagnóstico de anfitrión

### Comandos añadidos a la tabla 1.3

| Tipo | Comando | Función general |
|------|---------|-----------------|
| apps | apps | Ciclo de vida de apps |
| ia | iarouter | Modelos locales o remotos |
| red | red | Diagnóstico de red del anfitrión |
| tareas | hilos | Vista de automáticas |
| tareas | tareas | GTD local |

### Normas añadidas

- Un comando de app no sobrescribe un comando de sistema.
- Prioridad de resolución: sistema > app sistema > app usuario > user_.
- Mini-moslib solo con `app_dir` de esa app.
- iarouter no llama a red si `enabled` es false.
- Las claves de IA no se imprimen.
- Tras `update`, los módulos de la sesión no se sustituyen solos (`update reiniciar`).

### Estado de la reserva «campaña 07»

El código de 7.1–7.3 **existe** en el árbol. El apartado «Capacidades en curso» de 1.3 queda **cerrado como implementado**. Siguen fuera: malla P2P, tienda remota firmada, DepManager geo, cliente web de X.

### Núcleo añadido

| Nivel 1 | Nivel 2 | Nivel 3 | Descripción |
|---------|---------|---------|-------------|
| moslib/ | core/ | apps.py | Registro de apps |
| moslib/ | core/ | tasks.py | Tareas |
| moslib/ | core/ | ia_router.py | Fachada de modelos |
| moslib/ | core/ | ia_keys.py | Claves |
| moslib/ | core/ | secreto.py | Wrap HMAC |
| moslib/ | core/ | ia_share.py | LAN / publicar |
| moslib/ | core/ | ia_bridge.py | Puente :17337 |
| moslib/ | core/ | ia_check.py | Diagnósticos |
| moslib/ | core/ | entorno.py | Perfil de entorno |
| apps/ | dev/ | | App de desarrollo versionada |
